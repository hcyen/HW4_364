...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/hcyenHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
#search_gifs = db.Table('search_gifs', db.Column('search_id', db.Integer, db.ForeignKey('searchterms.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
#it is weird that the db.column 'searches_id' has to be in plural form. if you use search_id, it errors out saying no foreign key and it suggests you to use plural form
#same to the db.column 'gifs_id'. you could not use gif_id.
search_gifs = db.Table('search_gifs', db.Column('searches_id', db.Integer, db.ForeignKey('searchterms.id')), db.Column('gifs_id', db.Integer, db.ForeignKey('gifs.id')))
...
class User(UserMixin, db.Model):
    ...
    personalgifcollections = db.relationship('PersonalGifCollection', backref='User')
...
def get_gifs_from_giphy(search_string):
    ...
    #params_diction["api_key"] = giphy_api_key.api_key
    ...
    #pprint(python_obj)
    #http://api.giphy.com/v1/gifs/search?api_key=dc6zaTOxFJmzC&q=quora
    #print(python_obj['data'][0]['images']['fixed_height']['url'])
    
    return python_obj['data']
    ...
    ##https://www.quora.com/Exactly-how-can-you-work-with-API-with-the-Python-language-I-dont-understand-what-to-look-for
    ##contents = urllib2.urlopen("http://api.giphy.com/v1/gifs/search?q=quora&api_key=dc6zaTOxFJmzC").read() 
...
def get_gif_by_id(id):
    ...
    #g = db_session.query(Gif).filter_by(id=id).first()
    return g
...
def get_or_create_gif(title, url):
    ...
    #g = db_session.query(Gif).filter_by(title=title).first()
    if not g:
...
def get_or_create_search_term(term):
    ...
    if not search_term:
        ...
        for g in gif_list:
            #if 'images' in item: #if the key exists in the dictionary

            gif = get_or_create_gif(title= g['title'], url = g['embed_url'])
                #g = get_or_create_gif(title = g['title'], url = g['embed_url'])
                #print(python_obj['data'][0]['images']['fixed_height']['url'])
...
def get_or_create_collection( name, current_user, gif_list=[]):
    ...
    #personalgif_collection = db.session.query(PersonalGifCollection).filter_by(name=name, userid=current_user.id).first()
    personalgif_collection = PersonalGifCollection.query.filter_by(name=name, userid=current_user.id).first()
    # If it does not exist in the database yet, this function should create a new SearchTerm instance.
    if not personalgif_collection:
        personalgif_collection = PersonalGifCollection(name=name, userid=current_user.id, gifs=gif_list)
        for gif in gif_list:
            personalgif_collection.gifs.append(gif)

        db.session.add(personalgif_collection)
        db.session.commit()
        
        return personalgif_collection
    else:
...
def create_collection():
    ...
    if request.method == 'POST':
        ...
        picked = form.gif_picks.data
        gif_list = []
        for p in picked:
            gif_list.append(get_gif_by_id(int(p)))
        get_or_create_collection(form.name.data, current_user, gif_list)
        #return redirect(url_for('collections'))
        return "collection created"
        '''
        if  form.validate_on_submit(): #for some reason, this if condition prevent the data to be saved
            picked = form.gif_picks.data
            gif_list = []
            for p in picked:
            	gif_list.append(get_gif_by_id(int(p)))
...
def collections():
    ...
    pcollection  = PersonalGifCollection.query.filter_by(userid=current_user.id).all()
    return render_template('collections.html', collections=pcollection)
# Provided
